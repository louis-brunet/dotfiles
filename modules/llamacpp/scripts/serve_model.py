...
def proxy_tcp(client_socket: socket.socket, target_host: str, target_port: int) -> None:
    with (
        client_socket as client_socket,
        socket.socket(socket.AF_INET, socket.SOCK_STREAM) as target_socket,
    ):
        try:
...
def start_llama_server(cmd: list[str]) -> int:
    ...
    if child_server_id == 0:
        ...
        log(f"STARTED llama-server child process, pid: {child_server_id}")
...
def main() -> None:
    ...
    def stop_llama_server_and_proxies(
        ...
        try:
            for child_proxy_pid in proxy_pids:
                log(f"stopping child proxy with pid {child_proxy_pid}")
        ...
        try:
            if server_pid:
                log(f"stopping child server with pid {server_pid}")
    ...
    running_server_pid: int | None = start_llama_server(server_cmd)
