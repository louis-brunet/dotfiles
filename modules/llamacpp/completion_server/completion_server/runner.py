...
def _run_completion(
    ...
    logger = logging.getLogger(f"_run_completion.pid.{multiprocessing.current_process().pid}")
    llm: Llama | None = None
    pending_response_queue: (
        queue.Queue[None | CreateCompletionResponse | Iterator[CreateCompletionStreamResponse]]
        | None
    ) = None

    def force_terminate(signum, frame):
        logger.error(f"Received signal {signal.strsignal(signum)} ({signum}), terminating...")
