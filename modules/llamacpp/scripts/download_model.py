...
def run_cmd(cmd_args: list[str]):
    ...
    try:
        ...
        print(
            "ERROR running command '{}'. Exit code {}".format(" ".join(cmd_args), e.returncode),
            file=sys.stderr,
        )
...
def main():
    ...
    print(f"Downloaded files from {hf_repo} to {downloaded_directory}")

    if needs_merge:
        merge_model(downloaded_directory, output_file_path)
        print(f"Merged model files from {downloaded_directory} into {output_file_path}")
    else:
        files = os.listdir(downloaded_directory)
        if len(files) != 1:
            raise ValueError(
                f"Expected one file in the downloaded directory, but found {len(files)}"
            )
        downloaded_model = os.path.join(downloaded_directory, files[0])
        shutil.copyfile(downloaded_model, output_file_path)
        print(f"Copied file {downloaded_model} to {output_file_path}")
