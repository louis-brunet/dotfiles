...
#
#
# # # FIXME: the following script is adapted from the broken (as of 2025-06-12) script:
# # # https://github.com/sst/opencode/blob/c554430c59171e2dbec4b4d311e5f7d5a8107d0b/install
# # # This whole file should eventually be replaced with one of the following lines:
# # # npm install -g opencode
# # # curl -fsSL https://opencode.ai/install | VERSION="$OPENCODE_VERSION" bash
# #
# # # [[ -z "$OPENCODE_VERSION" ]] && {
# # #     echo "The latest (v0.0.53) version is "
# # #     exit 1
# # # }
# #
# # #!/usr/bin/env bash
...
# # case "$filename" in
# #     *"-linux-"*)
# #         [[ "$arch" == "x64" || "$arch" == "arm64" || "$arch" == "i386" ]] || exit 1
...
# #     print_message info "Downloading ${ORANGE}opencode ${GREEN}version: ${YELLOW}$specific_version ${GREEN} from ${url}..."
# #     mkdir -p opencodetmp && cd opencodetmp
# #
# #     # curl -# -L "$url" | tar xz
# #     download_filename="tmp_archive.zip"
# #     curl -# -L "$url" -o "${download_filename}" && unzip "$download_filename"
# #
